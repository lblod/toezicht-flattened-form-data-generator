...
export function createSubmissionForQuery(uri) {
  return `
    ...
    WHERE {
      ...
      ?logicalFile
        dct:type <http://data.lblod.gift/concepts/form-data-file-type> .
...
export function insertFormDataQuery({ uri, uuid, submission, properties }) {
  ...
  if (submission.isStatusSend) {
    statusSendTriple = `
     ${sparqlEscapeUri(uri)}
      ext:formSubmissionStatus ${sparqlEscapeUri(STATUS_SEND_CONCEPT)}.
    `;
  }
  ...
  return `
    ...
    INSERT {
      GRAPH ?g {
        ${sparqlEscapeUri(uri)}
          a ${sparqlEscapeUri(MELDING('FormData').value)} ;
          mu:uuid ${sparqlEscapeString(uuid)} .
        ${properties.map((property) => property.toNT(uri)).join('\n    ')}
        ${sparqlEscapeUri(uri)}
          ${sparqlEscapeUri(PROV('hadPrimarySource').value)}
            ${sparqlEscapeUri(submission.ttl.uri)} .
        ${sparqlEscapeUri(submission.uri)}
          ${sparqlEscapeUri(PROV('generated').value)} ${sparqlEscapeUri(uri)} .
        ${statusSendTriple || ''}
      }
    } WHERE {
      GRAPH ?g {
