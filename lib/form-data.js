...
export class FormData {
  ...
  async flatten() {
    const result = await query(
      completeFormDataFromSubmissionQuery(this.submission.uri)
    );
    const isNew = result.results.bindings.length == 0;
    if (isNew) {
      console.log(
        `No form data found for submission <${this.submission.uri}>. Going to create a  new flattened form data resource.`
      );
      this.uuid = uuid();
    ...
    const submissionIsStatusSend = await query(
      askSubmissionSendStatus(this.submission.uri)
    );
