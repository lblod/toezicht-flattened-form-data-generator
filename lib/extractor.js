...
import { retrieveCodeList, DECISION_TYPES, REGULATION_TYPES, TAX_TYPES, ADOPTION_TYPES, MUNICIPAL_ROAD_PROCEDURE_TYPES } from '../util/codelist';
...
const EXTRACTORS = [
  ...
  ({ graph, base }) => extract({ graph, base, find: RDFS('seeAlso') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPAuthenticSource') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPCorrectionType') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPGoal') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPExplanationType') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPDescription') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPClarification') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPClimateTableDate') }),
  ({ graph, base }) => extract({ graph, base, find: LBLOD_BESLUIT('LEKPHouseholds') }),
